...
function nextBid({ highBid, startingBid, estimateLow }) {
  const anchor = highBid ?? startingBid ?? estimateLow ?? null;
  if (anchor == null) return null;
  if (highBid == null) return Number(anchor);
...
function validateBid({ amount, highBid, startingBid, estimateLow, status, userId, highBidderId }) {
  ...
  const text = String(amount == null ? '' : amount).trim().replace(/,/g, '');
  ...
  const increment = incrementMinor(highBid != null ? minorUnits(highBid) : anchorMinor);
  const minMinor = highBid != null ? minorUnits(highBid) + increment : anchorMinor;
  ...
  const roundedMinor = highBid != null
...
async function placeBid({ userId, lotId, amount }) {
  return withTransaction(async (client) => {
    const lotResult = await client.query(
      `SELECT l.id, l.title, l.starting_bid, l.estimate_low, a.status
