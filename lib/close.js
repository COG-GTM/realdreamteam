...
// Highest amount wins; ties go to the earliest placed_at, then the lower id.
// Returns null when there are no bids. Pure, so it is unit-tested without a database.
function pickWinner(bids) {
  ...
  for (const bid of bids) {
    if (!winner) {
      ...
      const bidTime = new Date(bid.placed_at).getTime();
      const winnerTime = new Date(winner.placed_at).getTime();
      const bidId = bid.bid_id ?? bid.id;
      const winnerId = winner.bid_id ?? winner.id;
      if (bidTime < winnerTime || (bidTime === winnerTime && Number(bidId) < Number(winnerId))) {
        winner = bid;
      }
    }
  }
  ...
  return withTransaction(async (client) => {
    ...
    for (const lot of lots) {
      const bids = (await client.query(
        `SELECT b.id, b.user_id, b.amount, b.placed_at, u.name, u.shadow
      ...
      await logActivity(client, {
        kind: 'sold',
        actorUserId: winner.user_id,
        lotId: lot.id,
        auctionId,
        amount: winner.amount
      });
      const bidderIds = [...new Set(bids.filter((bid) => !bid.shadow).map((bid) => bid.user_id))];
    ...
    await logActivity(client, { kind: 'closed', auctionId });
