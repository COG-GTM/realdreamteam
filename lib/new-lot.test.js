...
test('validateLot rejects invalid numbers and inverted estimates', () => {
  assert.ok(validateLot({ ...good, lot_number: '7a' }).errors);
  assert.equal(validateLot({ ...good, estimate_low: '10.50' }).errors, undefined);
  assert.ok(validateLot({ ...good, estimate_low: '10.555' }).errors);
