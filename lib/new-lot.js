...
async function createLot(fields) {
  ...
  const result = await withTransaction(async (client) => {
    ...
    const checkedWithCategories = validateLot(fields, activeCategories);
    if (checkedWithCategories.errors) {
      const error = new Error(checkedWithCategories.errors.join(' '));
      error.status = 400;
      throw error;
    }
    const transactionValues = checkedWithCategories.lot;
    const auction = (await client.query(
      'SELECT id, title, location, starts_at, status FROM auctions WHERE id = $1',
      [transactionValues.auction_id]
    )).rows[0];
    ...
    const lot = (await client.query(
      ...
      [
        transactionValues.auction_id, transactionValues.lot_number, transactionValues.title, transactionValues.artist, transactionValues.category,
        transactionValues.description, transactionValues.currency, transactionValues.estimate_low, transactionValues.estimate_high,
        transactionValues.starting_bid, transactionValues.source_url
      ]
    )).rows[0];

    if (transactionValues.image_url) {
      await client.query(
        'INSERT INTO lot_images (lot_id, position, url, credit) VALUES ($1, 1, $2, $3)',
        [lot.id, transactionValues.image_url, transactionValues.image_credit]
      );
    }
