...
function validateLot(fields = {}, list = categories) {
  ...
  const lot = {
    ...
    estimate_low: optionalMoney(fields.estimate_low),
    estimate_high: optionalMoney(fields.estimate_high),
    starting_bid: optionalMoney(fields.starting_bid),
  ...
  if (Number.isNaN(lot.lot_number)) errors.push('lot number must be a whole number.');
  for (const name of ['estimate_low', 'estimate_high', 'starting_bid']) {
    if (Number.isNaN(lot[name])) errors.push(`${name.replace('_', ' ')} must be an amount with up to 2 decimals.`);
  }
  for (const name of ['lot_number', 'estimate_low', 'estimate_high', 'starting_bid']) {
    if (Number.isNaN(lot[name])) {
      continue;
    }
    if (lot[name] !== null && lot[name] < 0) {
      errors.push(`${name.replace('_', ' ')} cannot be negative.`);
    }
  }
...
async function createLot(fields) {
  const result = await withTransaction(async (client) => {
    ...
    await logActivity(client, { kind: 'new_lot', lotId: lot.id, auctionId: lot.auction_id });
...
async function notifyMatches(client, lot) {
  const users = (await client.query(
    `SELECT u.id, u.name, p.categories, p.artists, p.keywords
     ...
     WHERE u.banned = false AND u.shadow = false`
  )).rows;
  const matched = [];
