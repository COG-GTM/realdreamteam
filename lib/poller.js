...
const { query, withTransaction, seedIfEmpty } = require('../db/db');
...
async function runOnce() {
  ...
  // Open pass: status flip and its activity rows in one transaction, with its
  // own error handling so a failure here never blocks the close pass below.
  try {
    await withTransaction(async (client) => {
      ...
      for (const auction of opened) {
        await logActivity(client, { kind: 'opened', auctionId: auction.id });
      }
    });
  } catch (error) {
