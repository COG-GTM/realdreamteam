...
const { createLot } = require('../new-lot');
const { closeAuction } = require('../close');
const { availableBankLots, calendarCheck } = require('./recycle');
...
function pickLotFor(shadow, ctx, rng, { forBid = true } = {}) {
  const entries = [];
  for (const lot of ctx.lots) {
    if (forBid && lot.high_bidder_id != null && Number(lot.high_bidder_id) === Number(shadow.id)) continue;
    const weight = weightForLot(lot, { ...ctx, preferences: personaPrefs(shadow), persona: shadow.persona }, rng);
    if (weight <= 0) continue;
    if (forBid && lot.high_bidder_id != null && !lot.high_bidder_shadow) {
      const ok = fairness({
...
async function actBid(shadow, ctx, rng, log) {
  const lot = pickLotFor(shadow, ctx, rng, { forBid: true });
...
async function actFavorite(shadow, ctx, rng, log) {
  const lot = pickLotFor(shadow, ctx, rng, { forBid: false });
...
// §6.4: publish the longest-unseen bank lot into a random open auction via the
// admin createLot seam — matching humans get new_lot notifications and the
// Live pane shows the activity row.
async function actPublishLot(ctx, rng, log) {
  const auctions = (await query(
    "SELECT id, title FROM auctions WHERE status = 'open' ORDER BY id"
  )).rows;
  if (!auctions.length) return { action: 'publish_lot', skipped: 'no open auction' };
  const bank = await availableBankLots({ query });
  if (!bank.length) return { action: 'publish_lot', skipped: 'no bank lots left' };
  const seedLot = bank[0];
  const auction = rng.pick(auctions);
  const nextNumber = (await query(
    'SELECT COALESCE(MAX(lot_number), 0) + 1 AS n FROM lots WHERE auction_id = $1',
    [auction.id]
  )).rows[0].n;
  const image = Array.isArray(seedLot.images) ? seedLot.images[0] : null;
  const { lot, notified } = await createLot({
    auction_id: auction.id,
    lot_number: nextNumber,
    title: seedLot.title,
    artist: seedLot.artist,
    category: seedLot.category,
    description: seedLot.description,
    currency: seedLot.currency,
    estimate_low: seedLot.estimate_low,
    estimate_high: seedLot.estimate_high,
    starting_bid: seedLot.starting_bid,
    source_url: seedLot.source_url,
    image_url: image && image.url,
    image_credit: image && image.credit
  });
  log(`[sim] published bank lot ${lot.id} ("${seedLot.title}") into "${auction.title}" — ${notified} notified`);
  return { action: 'publish_lot', lotId: lot.id };
}

// §6.4: an open auction inside its last 30 minutes closes right now — the
// close seam writes the sold/closed activity and notifications.
async function actCloseEarly(ctx, rng, log) {
  ...
  if (!auction) return { action: 'close_early', skipped: 'nothing closing soon' };
  const result = await closeAuction(auction.id);
  log(`[sim] closed "${auction.title}" early: ${result.lotsSold} sold, ${result.lotsUnsold} unsold`);
  return { action: 'close_early', auctionId: auction.id };
}
...
async function runTick({ rng, now = new Date(), presence, log = console.log }) {
  ...
  // Auction-level actions need neither shadows nor open lots.
  if (action === 'publish_lot') return actPublishLot(ctx, rng, log);
  if (action === 'close_early') return actCloseEarly(ctx, rng, log);
