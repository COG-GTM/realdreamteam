...
let lastActionAt = 0; // epoch ms of the last executed (non-skipped) action
...
async function loop() {
  while (running) {
    ...
    const waking = !awake;
    awake = true;
    // Wake-up burst: the site would look frozen otherwise (§4). Skip it when
    // an action ran in the last 10 min — we weren't really asleep for long.
    if (waking && Date.now() - lastActionAt > 10 * 60 * 1000) {
