...
// One tick that can never kill the loop: tick errors are logged and skipped.
  ...
  while (running) {
    ...
    if (waking) {
      ...
      for (let i = 0; i < burst && running; i += 1) {
        await safeTick();
        nextTickAt = Date.now() + jitter(2, 5);
        await sleep(jitter(2, 5));
      }
      continue;
    }
    await safeTick();
    nextTickAt = Date.now() + jitter(MIN_S(), MAX_S());
    await sleep(jitter(MIN_S(), MAX_S()));
  }
}
...
function start() {
  ...
  running = true;
  loop()
    .catch((error) => console.error(`[sim] ${error.message}`))
    .then(() => {
      const wasRunning = running;
      running = false;
      // An unexpected exit would silence the simulator until restart; retry.
