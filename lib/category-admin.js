...
async function integrityReport(client = null) {
  const executor = client || query;
  const names = (await executor('SELECT name FROM categories')).rows.map((row) => row.name);
  const lots = (await executor('SELECT id, title, category FROM lots')).rows;
  const preferences = (await executor('SELECT user_id, categories FROM preferences')).rows;
